...
class Clientify_Endpoint {
    ...
    function gat_list_abandoned_cart($params){
        ...
        if (is_plugin_active('woo-cart-abandonment-recovery/woo-cart-abandonment-recovery.php')) {
            ...
            $total = $wpdb->get_results("SELECT wcac.cartflows_id FROM ". $wpdb->prefix . "clientify_abandoned_cart wcac WHERE DATE(date_add)  ".$date_null." group by wcac.cartflows_id ");   
            $order_ids = "SELECT wcac.cartflows_id FROM ". $wpdb->prefix . "clientify_abandoned_cart wcac WHERE DATE(date_add)  ".$date_null." group by wcac.cartflows_id ".$limit;
            ...
            foreach ( $cookie_carts as $cookie_carts_results => $cookie_cart_id ) {
                $cart = $wpdb->get_row('SELECT session_id  FROM ' . $wpdb->prefix . 'cartflows_ca_cart_abandonment WHERE id="'.$cookie_cart_id->cartflows_id.'"');
                $carts[] = $cart->session_id;		
            }
