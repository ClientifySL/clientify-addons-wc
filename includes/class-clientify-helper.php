...
define( 'CLIENTIFY_CART_ABANDONMENT_TABLE', 'clientify_ca_cart_abandonment' );
/**
 * Cart abandonment tracking class.
 */
class Clientify_Helper {
	...
	// 	global $wpdb;
	// 	$cart_abandonment_table_name = $wpdb->prefix . CLIENTIFY_CART_ABANDONMENT_TABLE;
	...
	public function get_checkout_details( $wcf_session_id ) {
		global $wpdb;
		$cart_abandonment_table = $wpdb->prefix . CLIENTIFY_CART_ABANDONMENT_TABLE;
		// Can't use placeholders for table/column names, it will be wrapped by a single quote (') instead of a backquote (`).
		$result = $wpdb->get_row(
	...
	function delete_old_logs($params) {
		...
		if ($resultado === false) {
			...
			return new WP_REST_Response([
				'message' => 'Error al eliminar los logs antiguos.',
			], 500); // 500 Internal Server Error
		} elseif ($resultado === 0) {
			// No se eliminaron registros porque no había logs antiguos
			return new WP_REST_Response([
				'message' => 'No se encontraron logs antiguos para eliminar.',
			], 404); // 404 Not Found
		} else {
			// Registros eliminados con éxito
			return new WP_REST_Response([
				'message' => 'Logs antiguos eliminados correctamente.',
				'deleted_rows' => $resultado,
			], 200); // 200 OK
		}
